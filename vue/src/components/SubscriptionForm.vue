  ...
  <div class="card mt-8">
    ...
    <div class="mt-4 md:mt-12 mb-12">
      <div v-if="subscriptionStatus == SubscriptionStatus.LoadedWithSubs" class="text-center">
        ...
        <p v-if="loading == LoadingStatus.Loaded && !inUpdateMode" class="mb-4 text-xs text-slate-500 dark:text-slate-200">Last updated: {{ updateDate }} </p>

      </div>

      <div v-if="subscriptionStatus == SubscriptionStatus.LoadedNoSubs" class="update-sub">
        <p class="mb-4">
          <span v-if="topicsReminder && !canSubscribe" class="text-red-600 dark:text-red-400 mb-4">Select at least one topic</span>
      ...
      <h3 v-if="loading == LoadingStatus.Error" class="mt-8 mb-8 text-center text-slate-500 dark:text-slate-200">Sorry, something went wrong. Try again.</h3>
    </div>
  </div>
</template>
